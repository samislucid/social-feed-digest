...
from digest.render import DEGRADED_SUBJECT_TAG, Digest, build_email, render_html, render_markdown
...
def test_html_mirrors_the_channel_shape(base_profile):
    ...
    assert "Drafts for your account" in html
...
def test_build_email_keeps_subject_and_both_parts(base_profile):
    digest = _digest(base_profile)
    message = build_email(digest, render_markdown(digest), render_html(digest), "digest@localhost")
